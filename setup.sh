...
# Load existing .env values as defaults so users see and can confirm current settings
...
if [[ -f .env ]]; then
  ...
  info "No .env found — please enter your configuration values."
fi
...
[[ -f .env ]] && mv .env ".env.backup.$(date +%Y%m%d%H%M%S)" && warn "Previous .env backed up."
